...
export default function TextToImage() {
    ...
    const [dynamicParams, setDynamicParams] = useState({}); // New: Store dynamic parameter values
            ...
            case 'select':
                return (
                    <div key={param.key} className="space-y-2">
                        <label className="text-[10px] font-black uppercase text-gray-500 tracking-[0.15em] flex items-center gap-2 px-2">
        ...
        <GenerationWrapper type="image">
            {({ handleGenerate, isExhausted, freeGenerationsLeft }) => {
                const onGenerateClick = async () => {
                    ...
                    if (!selectedModel) {
                        toast.error(t("generator.errors.modelRequired"));
                        return;
                    }
                    ...
                    <div className="space-y-4">
                        ...
                        <div className={`relative group transition-all duration-300 ${isUsingTemplate && templateData.promptEditable === false ? 'opacity-80' : ''}`}>
                            <textarea
                                ...
                                className={`w-full h-48 p-6 rounded-[2.5rem] bg-white/5 border border-white/10 text-white placeholder-gray-500 focus:border-purple-500/50 focus:bg-white/[0.07] outline-none transition-all duration-500 resize-none text-sm leading-relaxed font-medium ${isUsingTemplate && templateData.promptEditable === false ? 'cursor-not-allowed bg-white/[0.02]' : ''
                                    }`}
                            />
                ...
                const handleApplyHistory = (item) => {
                    ...
                    // if (item.aspectRatio) setSelectedAspect(item.aspectRatio); // Legacy
                    // We might need to handle restoring dynamic params too, but that depends on history format.
